...
**RDE26-MOP は、IEEE WCCI 2026 / IEEE CEC 2026 の「精度と速度を考慮した数値最適化コンペティション」における Competition 3-BC-MOPs トラックの公式優勝・第 1 位のアルゴリズムです。**

本リポジトリは、有界制約多目的最適化トラック向けの最終版オープンソースアルゴリズムパッケージを提供します。
...
- 🏆 CEC 2026 BC-MOPs トラックの公式優勝アルゴリズムです。
...
| `code/rde26-mop/main.py` | トラック専用の実行エントリ |
| `code/rde26-mop/src/` | 最終アルゴリズムのソースコード |
...
生成された出力は `code/rde26-mop/outputs/` に書き込まれます。
