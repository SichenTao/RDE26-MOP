    ...
    methods
        ...
        function PopObj = CalObj(obj, PopDec)
            ...
            for i = 1:N
                ...
                for n=nobj:1:nvar
                    if mod(n,5)==0
                        g = g + 2*sin(PopDec(i,1)*pi)*(abs(-0.9*(PopDec(i,n) - tmp1).^2) +  abs(PopDec(i,n) - tmp1).^(0.6));
                    else
                        g = g + 2*sin(PopDec(i,1)*pi)*(abs(-0.9*(PopDec(i,n) - 0.5).^2) +  abs(PopDec(i,n) - 0.5).^(0.6));
                    end
                end
                g = g*10;
                tmp2 = 1;
